...
def _extract_conn_attributes(conn_kwargs, db_sem_conv_opt_in_mode, http_sem_conv_opt_in_mode):
    ...
    # valkey-py may pass db=None explicitly (e.g. from_url); treat that as 0.
...
def _set_connection_attributes(
    span: Span,
    conn: ValkeyInstance | AsyncValkeyInstance,
...
def _build_span_name(instance: ValkeyInstance | AsyncValkeyInstance, cmd_args: tuple[Any, ...]) -> str:
...
def _build_span_meta_data_for_pipeline(
    ...
    try:
        # valkey-py 6+ ClusterPipeline no longer updates ``command_stack``;
        ...
        # ``_command_stack`` for non-cluster pipelines and older valkey-py.
